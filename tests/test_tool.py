...
class TestCliValidate(unittest.TestCase):
    ...
    def test_input_int_valid(self, mock_input):
        tool = Tool()
        value = tool.input_int(message='Value: ')
    ...
    def test_range_input_valid(self, mock_input):
        tool = Tool()
        result = tool.range_input(from_value=1, to_value=4, message="Ingrese un número entre 1 y 4: ")
    ...
    def test_range_input_invalid(self, mock_input):
        tool = Tool()
        with patch('sys.stdout', new_callable=StringIO) as mock_stdout:
            value = tool.range_input(from_value=1, to_value=4, message="Ingrese un número entre 1 y 4: ")
        self.assertEqual(value, 3)

class TestCliMenu(unittest.TestCase):
    def test_menu_print(self):
        tool = Tool()
        print_output = tool.menu(type='menu')
        self.assertEqual(
            print_output,
            '''    Menu
1) Ver tablero
...
4) Ver Puntuaciones
    ...
    def test_submenu_print(self):
        tool = Tool()
        print_output = tool.menu(type='submenu')
        self.assertEqual(
            print_output,
